...
def interactive_int_to_rom():
    converter_run = True
    print(logo)
    num = int(input("Type a number to convert between 1 and 3999.\n"))
    while converter_run:
        while num < 1 or num > 3999:
            num = int(input("Type a number to convert between 1 and 3999.\n"))
        else:
            print(f"{num} in Roman Numerals is: {int_to_rom(num)}")
            num = int(input("Would you like to convert another number?\n"))

interactive_int_to_rom()
